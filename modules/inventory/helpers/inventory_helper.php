 ...
 * @return  array
 */
function get_all_products_attachments($id){
	$CI = &get_instance();

	$attachments = [];
	$attachments['products'] = [];

	$permission_estimates_view = has_permission('products', '', 'view');
	$permission_estimates_own = has_permission('products', '', 'view_own');
...
function get_product_discussions_language_array(){
	$lang = [
...
function handle_product_file_uploads($product_id){
	$CI = &get_instance();
	$filesIDS = [];
	$errors = [];
	...
	if (isset($_FILES['file']['name'])
		...
		hooks()->do_action('before_upload_product_attachment', $product_id);
		...
		$path = FCPATH . 'uploads/products' . '/' . $product_id . '/';
		if (!file_exists($path)) {
		...
		for ($i = 0; $i < count($_FILES['file']['name']); $i++) {
			...
			if (!empty($tmpFilePath) && $tmpFilePath != '') {
				...
				if (move_uploaded_file($tmpFilePath, $newFilePath)) {
					...
					$CI->db->insert(db_prefix() . 'product_files', $data);
	...
	if (count($filesIDS) > 0) {
		...
		//$CI->products_model->new_product_file_notification($filesIDS[$lastFileID], $product_id);
	}
...
function product_file_url($file, $preview = false){
	$path = 'uploads/products/' . $file['product_id'] . '/';
	...
	if (!empty($file['external']) && !empty($file['thumbnail_link'])) {
		...
		if ($preview) {
			...
			if (file_exists($thumbPath)) {
				$url = base_url('uploads/products/' . $file['product_id'] . '/' . $fname . '_thumb.' . $fext);
			}
		}
	}
