...
Dropzone.options.mestimateExpenseForm = false;
...
$(function () {
    ...
    var $discussionsContentEditable = $('#mestimate_file_data, #discussion-comments');
    ...
    $('body').on('show.bs.modal', '._mestimate_file', function () {
        discussion_comments('#mestimate-file-discussion', discussion_id, 'file')
    });
    ...
    initDataTable('.table-contracts', admin_url + 'contracts/table?mestimate_id=' + mestimate_id, undefined, undefined, ContractsServerParams, [6, 'desc']);

    if ($('#timesheetsChart').length > 0 && typeof (mestimate_overview_chart) != 'undefined') {
        var chartOptions = {
            type: 'bar',
            data: {},
            options: {
        ...
        chartOptions.data = mestimate_overview_chart.data;
    ...
    $('body').on('click', '.milestone-column .cpicker,.milestone-column .reset_milestone_color', function (e) {
        ...
        $.post(admin_url + 'mestimates/change_milestone_color', {
            color: color,
    ...
    appValidateForm($('#mestimate-expense-form'), {
        category: 'required',
        date: 'required',
        amount: 'required',
        currency: 'required'
    }, mestimateExpenseSubmitHandler);
    ...
    _table_api = initDataTable('.table-mestimate-expenses', admin_url + 'mestimates/expenses/' + mestimate_id, 'undefined', 'undefined', Expenses_ServerParams, [4, 'desc']);

    if (_table_api) {
        _table_api.column(0).visible(false, false).columns.adjust();
    }

    init_rel_tasks_table(mestimate_id, 'mestimate');
    initDataTable('.table-notes', admin_url + 'mestimates/notes/' + mestimate_id, [4], [4], 'undefined', [1, 'desc']);
    ...
    initDataTable('.table-timesheets', admin_url + 'mestimates/timesheets/' + mestimate_id, [8], [8], Timesheets_ServerParams, [3, 'desc']);
    initDataTable('.table-mestimate-discussions', admin_url + 'mestimates/discussions/' + mestimate_id, undefined, undefined, 'undefined', [1, 'desc']);
    ...
    $('#timesheet select[name="timesheet_task_id"]').on('change', function () {
        ...
        requestGet('mestimates/timesheet_task_assignees/' + _task_id + '/' + mestimate_id + '/' + staff_id).done(function (response) {
            select_staff.html(response);
            select_staff.selectpicker('refresh');
        });
    });

    $('body').on('change', '#mestimate_invoice_select_all_tasks,#mestimate_invoice_select_all_expenses', function () {
...
function saveProjectExternalFile(files, externalType) {
    $.post(admin_url + 'mestimates/add_external_file', {
        ...
        window.location.href = location.split('?')[0] + '?group=mestimate_files';
    });
}

function milestones_switch_view() {
    $('#milestones-table').toggleClass('hide');
    $('.mestimate-milestones-kanban').toggleClass('hide');
    if (!$.fn.DataTable.isDataTable('.table-milestones')) {
        initDataTable('.table-milestones', admin_url + 'mestimates/milestones/' + mestimate_id);
    }
}
...
function manage_discussion(form) {
    ...
    $.post(url, data).done(function (response) {
        ...
        $('.table-mestimate-discussions').DataTable().ajax.reload(null, false);
...
function mass_stop_timers(only_billable) {
    requestGetJSON('mestimates/mass_stop_timers/' + mestimate_id + '/' + only_billable).done(function (response) {
        alert_float(response.type, response.message);
        setTimeout(function () {
            $('body').find('.modal-backdrop').eq(0).remove();
            init_timers();
            reload_tasks_tables();
            pre_invoice_mestimate();
        }, 500);
    });
}

function pre_invoice_mestimate() {
    requestGet('mestimates/get_pre_invoice_mestimate_info/' + mestimate_id).done(function (response) {
        $('#pre_invoice_mestimate').html(response);
        $('#pre_invoice_mestimate_settings').modal('show');
    });
}

function invoice_mestimate(mestimate_id) {
    $('#pre_invoice_mestimate_settings').modal('hide');
    var data = {};
    ...
    $.post(admin_url + 'mestimates/get_invoice_mestimate_data/', data).done(function (response) {
        $('#invoice_mestimate').html(response);
        $('#invoice-mestimate-modal').modal({
            show: true,
            backdrop: 'static'
        });
    });
...
function delete_mestimate_discussion(id) {
    if (confirm_delete()) {
        requestGetJSON('mestimates/delete_discussion/' + id).done(function (response) {
...
function mestimateExpenseSubmitHandler(form) {
...
function mestimate_mark_as_modal(status_id, $mestimate_id, target) {
    $('#mark_tasks_finished_modal').modal('show');
    $('#mestimate_mark_status_confirm').attr('data-status-id', status_id);
    $('#mestimate_mark_status_confirm').attr('data-mestimate-id', mestimate_id);
    var $mestimateMarkedAsFinishedInput = $('#mestimate_marked_as_finished_email_to_contacts');
    if (status_id == 4) {
        if ($mestimateMarkedAsFinishedInput.length > 0) {
            $mestimateMarkedAsFinishedInput.parents('.mestimate_marked_as_finished').removeClass('hide');
        }
    } else {
        if ($mestimateMarkedAsFinishedInput.length > 0) {
            $mestimateMarkedAsFinishedInput.prop('checked', false);
            $mestimateMarkedAsFinishedInput.parents('.mestimate_marked_as_finished').addClass('hide');
        }
    }
...
function mestimate_files_bulk_action(e) {
    if (confirm_delete()) {
        ...
        var rows = $('.table-mestimate-files').find('tbody tr');
        $.each(rows, function () {
        ...
        setTimeout(function () {
            $.post(admin_url + 'mestimates/bulk_action_files', data).done(function () {
                window.location.reload();
            });
        }, 200);
    }
...
function confirm_mestimate_status_change(e) {
    var data = {};

    $(e).attr('disabled', true);

    data.mestimate_id = $(e).data('mestimate-id');
    data.status_id = $(e).data('status-id');

    if (data.status_id == 4) {
        var $mestimateMarkedAsFinishedInput = $('#mestimate_marked_as_finished_email_to_contacts');
        if ($mestimateMarkedAsFinishedInput.length > 0) {
            data.send_mestimate_marked_as_finished_email_to_contacts = $mestimateMarkedAsFinishedInput.prop('checked') === true ? 1 : 0;
        }
    }
    ...
    data.notify_mestimate_members_status_change = $('#notify_mestimate_members_status_change').prop('checked') === true ? 1 : 0;

    $.post(admin_url + 'mestimates/mark_as', data).done(function (response) {
        response = JSON.parse(response);
...
function milestones_kanban_update(ui, object) {
    if (object === ui.item.parent()[0]) {
        ...
        setTimeout(function () {
            $.post(admin_url + 'mestimates/update_task_milestone', data)
        }, 50);
    }
}

function milestones_kanban() {
    init_kanban('mestimates/milestones_kanban', milestones_kanban_update, '.mestimate-milestone', 445, 360, after_milestones_kanban);
}
...
function after_milestones_kanban() {
    $("#kan-ban").sortable({
        ...
        update: function (event, ui) {
            ...
            $.post(admin_url + 'mestimates/update_milestones_order', data);
        }
    });
...
// When marking task as complete if the staff in on mestimate milestones area, remove this task from milestone in case exists
function _maybe_remove_task_from_mestimate_milestone(task_id) {
    var $milestonesTasksWrappers = $('.milestone-column');
    if ($("body").hasClass('mestimate') && $milestonesTasksWrappers.length > 0) {
