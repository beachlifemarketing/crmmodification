...
<div class="modal-header task-single-header" data-task-single-id="<?php echo $task->id; ?>" id="modal_form_task"
        ...
        <div class="col-md-8 task-single-col-left">
         ...
         <?php echo _l('add_checklist_item'); ?>
            ...
            <div class="tasks-comments inline-block full-width simple-editor"<?php if (count($task->comments) == 0) {
                ...
                <button type="button" class="btn btn-info mtop10 pull-right hide" id="addTaskCommentBtn"
        ...
        <div class="col-md-4 task-single-col-right">
            ...
            <h5 class="no-mtop task-info-created">
                <?php if (($task->addedfrom != 0 && $task->addedfrom != get_staff_user_id()) || $task->is_added_from_contact == 1) { ?>
                    <small class="text-dark"><?php echo _l('task_created_by', '<span class="text-dark">' . ($task->is_added_from_contact == 0 ? get_staff_full_name($task->addedfrom) : get_contact_full_name($task->addedfrom)) . '</span>'); ?>
                        <i class="fa fa-clock-o" data-toggle="tooltip"
                           data-title="<?php echo _l('task_created_at', _dt($task->dateadded)); ?>"></i></small>
                    <br/>
                <?php } else { ?>
            ...
            $start_date_array = explode(" ", $value);

            $start_date_value = $start_date_array[0];
            ...
            <div id="div_timer">
                <div class="task-info task-single-inline-wrap task-info-start-date">
                    <h5><i class="fa task-info-icon fa-fw fa-lg fa-calendar-plus-o pull-left fa-margin"></i>
                        ...
                        <?php if (has_permission('tasks', '', 'edit') && $task->status != 5) { ?>
                            <input name="startdate" tabindex="-1" value="<?php echo _d($start_date_value); ?>"
                    ...
                    <input type="time" style="border: none; margin-left: 80px; font-size: 12px;background: none;text-decoration-line: underline;
                                    text-decoration-style: dotted;" onchange="updateDateTime()"
                           tabindex="-1" name="starttime" id="starttime" class="" value="<?php echo $start_time; ?>"
                           mdbInput/>
                </div>
                ...
                <?php
                if (isset($task)) {
                    $value = _d($task->duedate);
                } else if (isset($due_date)) {
                    $value = _d($due_date);
                } else {
                    $value = _d(date('Y-m-d'));
                }
                $due_date_array = explode(" ", $value);

                $due_date_value = $due_date_array[0];
                    ...
                    <h5>
                        ...
                        <?php if (has_permission('tasks', '', 'edit') && $task->status != 5) { ?>
                            <input
                                    name="duedate"
                                    tabindex="-1"
                                    value="<?php echo _d($due_date_value); ?>"
                                    id="task-single-duedate"
                                    class="datepicker pointer" onchange="updateDateTime()"
                                    autocomplete="off"
                                    style="border: none;
                                background: none;
                                text-decoration-line: underline;
                        ...
                        <?php } else { ?>
                            <?php echo _d($due_date_value); ?>
                        <?php } ?>
                    </h5>
                    <input type="time" style="border: none; margin-left: 80px; font-size: 12px;background: none;text-decoration-line: underline;
                                    text-decoration-style: dotted;" onchange="updateDateTime()"
                           tabindex="-1" name="duetime" id="duetime" class="" value="<?php echo $due_time; ?>"
                           mdbInput/>
            ...
            <hr class="task-info-separator"/>
            <div class="clearfix"></div>
            <h4 class="task-info-heading font-normal font-medium-xs"><i class="fa fa-user-o"
...
<script>
    ...
    function updateDateTime() {
        simpleAjaxPostUpload(
            admin_url + 'tasks/task_single_new_update?rtype=json',
            '#div_timer',
            function (res) {
            },
            function (res) {
