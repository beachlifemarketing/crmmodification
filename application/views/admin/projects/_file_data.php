    ...
    <div style="float: left">
        <?php if (isset($file_next)) { ?>
            <button type="button" class="modal-title" style="margin-right: 15px;"
                    onclick="view_project_file('<?= $file_next->id ?>', '<?= $file_next->project_id ?>'); return false;">
                <i class="fa fa-arrow-left"></i></button>
        <?php } ?>
        <!--bootstrap card with 3 horizontal images-->

        <?php if (isset($file_next) && is_image(PROJECT_ATTACHMENTS_FOLDER . $file_next->project_id . '/' . $file_next->file_name) || (!empty($file_next->external) && !empty($file_next->thumbnail_link))) {
            echo '<img onclick="view_project_file(\'' . $file_next->id . '\', \'' . $file_next->project_id . '\'); return false;" class="modal-title img-width-50 project-file-image" src="' . project_file_url(get_object_vars($file_next), true) . '" width="80">';
        } ?>

        <?php if (isset($file) && is_image(PROJECT_ATTACHMENTS_FOLDER . $file->project_id . '/' . $file->file_name) || (!empty($file->external) && !empty($file->thumbnail_link))) {
            echo '<img class="modal-title project-file-image" src="' . project_file_url(get_object_vars($file), true) . '" width="50" height="50" style="border:1px solid black">';
        } ?>

        <?php if (isset($file_previous) && is_image(PROJECT_ATTACHMENTS_FOLDER . $file_previous->project_id . '/' . $file_previous->file_name) || (!empty($file_previous->external) && !empty($file_previous->thumbnail_link))) {
            echo '<img onclick="view_project_file(\'' . $file_previous->id . '\', \'' . $file_previous->project_id . '\'); return false;" class="modal-title img-width-50 project-file-image" src="' . project_file_url(get_object_vars($file_previous), true) . '" width="80">';
        } ?>

        <?php if (isset($file_previous)) { ?>
            <button type="button" class="modal-title" style="margin-right: 15px;"
                    onclick="view_project_file('<?= $file_previous->id ?>', '<?= $file_previous->project_id ?>'); return false;">
                <i
                        class="fa fa-arrow-right"></i></button>
        <?php } ?>
    </div>
    <button type="button" class="btn btn-default"
            onclick="close_modal_manually('._project_file'); return false;"><?php echo _l('close'); ?></button>
</div>